...
function constantValue(constants, index) {
    const entry = constants[index];
    return entry && typeof entry === 'object' && Object.prototype.hasOwnProperty.call(entry, 'type') ? entry.value : entry;
}
...
function executeProgram(program, globals = {}) {
    ...
    function exec(id, parent, args) {
        ...
        for (let i = 0; i < def.params.length; i += 1) env.cells.set(constantValue(constants, def.params[i]), { value: args[i] });
        ...
        while (pc <= def.code.length) {
            ...
            else if (op === OPS.LOAD_VAR) stack.push(getVar(env, constantValue(constants, a)));
            else if (op === OPS.STORE_VAR) setVar(env, constantValue(constants, a), stack.pop());
