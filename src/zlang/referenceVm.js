...
function setVar(env, key, value) { for (let current = env; current; current = current.parent) { const cell = current.cells.get(key); if (cell) { cell.value = value; return; } } env.cells.set(key, { value }); }
function luaBinary(id, left, right) {
    switch (id) {
        ...
        case BIN['//']: return Math.floor(left / right); default: throw new Error(`Z reference VM: binary opcode ${id}`);
    }
}
...
function executeProgram(program, globals = {}) {
    ...
    function exec(id, parent, args) {
        ...
        while (pc <= def.code.length) {
            const ins = def.code[pc - 1]; const op = ins[0]; const a = ins[1]; const b = ins[2]; const d = ins[4]; pc += 1;
            ...
            else if (op === OPS.GET_MEMBER) { const obj = stack.pop(); stack.push(obj?.[constants[a]]); }
            else if (op === OPS.SET_MEMBER) { const value = stack.pop(); const obj = stack.pop(); obj[constants[a]] = value; }
            ...
            else if (op === OPS.CALL_METHOD || op === OPS.CALL_METHOD_MULTI) { const callArgs = []; for (let i = b - 1; i >= 0; i -= 1) callArgs[i] = stack.pop(); const obj = stack.pop(); const fn = obj?.[constants[a]]; const result = invoke(fn, [obj, ...callArgs]); stack.push(op === OPS.CALL_METHOD_MULTI ? result : result.values[0]); }
            ...
            else if (op === OPS.FOR_NUM_PREP) { const step = stack.pop(); const finish = stack.pop(); const start = stack.pop(); if (step === 0) throw new Error('Z reference VM: numeric for step is zero'); const frame = { key: constants[a], current: start, finish, step }; const keep = step > 0 ? start <= finish : start >= finish; if (!keep) pc = d; else { loops.push(frame); setVar(env, frame.key, start); } }
            else if (op === OPS.FOR_NUM_NEXT) { const frame = loops[loops.length - 1]; frame.current += frame.step; const keep = frame.step > 0 ? frame.current <= frame.finish : frame.current >= frame.finish; if (keep) setVar(env, frame.key, frame.current); else { loops.pop(); pc = d; } }
            else if (op === OPS.ITER_PREP) { const keys = []; for (let i = a - 1; i >= 0; i -= 1) keys[i] = stack.pop(); const iterator = stack.pop(); if (!isMulti(iterator) || iterator.values.length < 3) throw new Error('Z reference VM: iterator setup'); const [fn, state, control] = iterator.values; const first = invoke(fn, [state, control]); const frame = { fn, state, control: first.values[0], keys }; if (frame.control == null) pc = d; else { loops.push(frame); for (let i = 0; i < keys.length; i += 1) setVar(env, keys[i], first.values[i]); } }
            else if (op === OPS.ITER_NEXT) { const frame = loops[loops.length - 1]; const next = invoke(frame.fn, [frame.state, frame.control]); frame.control = next.values[0]; if (frame.control == null) { loops.pop(); pc = d; } else { for (let i = 0; i < frame.keys.length; i += 1) setVar(env, frame.keys[i], next.values[i]); pc = b; } }
    ...
    return exec(program.root, null, []).values[0];
}
