...
class FunctionBuilder {
    ...
    allocLocal(sourceName) {
        const key = `@${this.fn.id}:${this.localSerial++}:${sourceName}`;
        this.scopes[this.scopes.length - 1].set(sourceName, key);
        return key;
    }
    ...
    resolveLocal(name) {
        ...
        return this.parent ? this.parent.resolveLocal(name) : null;
    }

    resolve(name) {
        const local = this.resolveLocal(name);
        return local ? { local: true, key: local } : { local: false, key: name };
    }

    loadName(name) {
        const r = this.resolve(name);
        this.emit(r.local ? OPS.LOAD_VAR : OPS.LOAD_GLOBAL, this.program.string(r.key));
    }

    storeName(name) {
        const r = this.resolve(name);
        this.emit(r.local ? OPS.STORE_VAR : OPS.STORE_GLOBAL, this.program.string(r.key));
    }
    ...
    emitExpr(node, wantMulti = false) {
        ...
        switch (node.type) {
            ...
            case 'UnaryExpression':
                ...
                this.emitExpr(node.argument);
                this.emit(OPS.UNARY, UNARY[node.operator]);
                return;
            case 'BinaryExpression':
                ...
                this.emit(OPS.BIN, BIN[node.operator]);
                return;
            case 'LogicalExpression': {
                ...
                this.emit(OPS.POP);
                this.emitExpr(node.right, wantMulti);
                this.patch(branch, 1, this.here());
                return;
            }
            case 'MemberExpression':
                this.emitExpr(node.base);
                this.emit(OPS.GET_MEMBER, this.program.string(node.identifier.name));
                return;
            ...
            case 'TableConstructorExpression': {
                ...
                for (const field of node.fields || []) {
                    ...
                    if (field.type === 'TableKeyString') {
                        this.emit(OPS.PUSH_CONST, this.program.string(field.key.name));
                        this.emitExpr(field.value);
                    } else if (field.type === 'TableKey') {
                        this.emitExpr(field.key);
                        this.emitExpr(field.value);
                    } else if (field.type === 'TableValue') {
                        this.emit(OPS.PUSH_CONST, this.program.number(arrayIndex++));
                        this.emitExpr(field.value);
                    } else {
                        throw new Error(`Z table field no soportado: ${field.type}.`);
                    }
            ...
            case 'FunctionExpression':
                ...
                return;
            case 'CallExpression':
                this.emitCall(node, wantMulti);
                return;
            case 'TableCallExpression':
                this.emitCall({ type: 'CallExpression', base: node.base, arguments: node.arguments }, wantMulti);
                return;
            case 'StringCallExpression':
                this.emitCall({ type: 'CallExpression', base: node.base, arguments: [node.argument] }, wantMulti);
                return;
            default:
                throw new Error(`Z expression no soportada: ${node.type}.`);
        }
    }
    ...
    emitCall(node, wantMulti) {
        ...
        if (base && base.type === 'MemberExpression' && base.indexer === ':') {
            ...
            for (const arg of args) this.emitExpr(arg);
    ...
    emitLValueStore(node) {
        ...
        if (node.type === 'MemberExpression') {
            this.emitExpr(node.base);
            return this.emit(OPS.SET_MEMBER, this.program.string(node.identifier.name));
        }
        if (node.type === 'IndexExpression') {
            this.emitExpr(node.base);
            this.emitExpr(node.index);
            return this.emit(OPS.SET_INDEX);
        }
        throw new Error(`Z destino no soportado: ${node.type}.`);
    }
        ...
        switch (node.type) {
            case 'LocalStatement': {
                ...
                for (let i = keys.length - 1; i >= 0; i -= 1) {
                    if (i >= init.length) this.emit(OPS.PUSH_CONST, this.program.nil());
                    this.emit(OPS.STORE_VAR, this.program.string(keys[i]));
                }
                return;
            }
            case 'AssignmentStatement': {
                ...
                for (let i = 0; i < vars.length; i += 1) {
                    if (init[i]) this.emitExpr(init[i]);
                    else this.emit(OPS.PUSH_CONST, this.program.nil());
                    const key = this.allocTemp();
                    temps.push(key);
                    this.emit(OPS.STORE_VAR, this.program.string(key));
                }
                for (let i = 0; i < vars.length; i += 1) {
                    this.emit(OPS.LOAD_VAR, this.program.string(temps[i]));
                    this.emitLValueStore(vars[i]);
                }
                return;
            }
            case 'CallStatement':
                this.emitExpr(node.expression);
                this.emit(OPS.POP);
                return;
            case 'ReturnStatement': {
                ...
                if (args.length === 0) {
                    ...
                    this.emit(OPS.RETURN, 1);
                } else if (args.length === 1) {
                    this.emitExpr(args[0]);
                    this.emit(OPS.RETURN, 1);
                } else {
                    for (const arg of args) this.emitExpr(arg);
                    this.emit(OPS.RETURN_MULTI, args.length);
                }
                return;
            }
            case 'IfStatement': {
            ...
            case 'BreakStatement': {
                ...
                loop.breaks.push(this.emit(OPS.BREAK));
                return;
            }
            case 'ForNumericStatement': {
                ...
                this.pushScope();
                const key = this.allocLocal(node.variable.name);
                const prep = this.emit(OPS.FOR_NUM_PREP, this.program.string(key));
                const bodyStart = this.here();
                const loop = { breaks: [] };
                this.loopStack.push(loop);
                this.compileBlock(node.body, false);
                const next = this.emit(OPS.FOR_NUM_NEXT);
                this.emit(OPS.JUMP, bodyStart);
                const end = this.here();
                this.patch(prep, 4, end);
                this.patch(next, 4, end);
                for (const at of loop.breaks) this.patch(at, 1, end);
                this.loopStack.pop();
                this.popScope();
                return;
            ...
            case 'ForGenericStatement': {
                ...
                if (!vars.length || iterators.length !== 1) throw new Error('Z generic for soporta un solo iterador y al menos una variable.');
                this.emitExpr(iterators[0], true);
                this.pushScope();
                const keys = vars.map(v => {
                ...
                const prep = this.emit(OPS.ITER_PREP, keys.length, 0, 0, 0);
                const bodyStart = this.here();
                const loop = { breaks: [] };
                this.loopStack.push(loop);
                this.compileBlock(node.body, false);
                const next = this.emit(OPS.ITER_NEXT, 0, bodyStart, 0, 0);
                const end = this.here();
                this.patch(prep, 4, end);
                this.patch(next, 4, end);
                for (const at of loop.breaks) this.patch(at, 1, end);
            ...
            case 'FunctionDeclaration': {
                ...
                if (identifier.type === 'Identifier') {
                    if (node.isLocal) this.emit(OPS.STORE_VAR, this.program.string(localKey));
                    ...
                    throw new Error(`Z function target no soportado: ${identifier.type}.`);
                }
                return;
            }
            case 'GotoStatement':
                this.pendingGotos.push({ at: this.emit(OPS.JUMP), label: node.label.name });
                return;
            case 'LabelStatement':
                this.labels.set(node.label.name, this.here());
                return;
            case 'EmptyStatement':
                this.emit(OPS.NOP);
                return;
            default:
                throw new Error(`Z statement no soportada: ${node.type}.`);
        }
    }
