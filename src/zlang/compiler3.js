...
class FunctionBuilder {
    ...
    allocLocal(sourceName) { const key = `@${this.fn.id}:${this.localSerial++}:${sourceName}`; this.scopes[this.scopes.length - 1].set(sourceName, key); return key; }
    ...
    emitExpr(node, wantMulti = false) {
        ...
        switch (node.type) {
            ...
            case 'UnaryExpression':
                if (UNARY[node.operator] === undefined) throw new Error(`Z no soporta operador unary ${node.operator}.`);
                this.emitExpr(node.argument); this.emit(OPS.UNARY, UNARY[node.operator]); return;
            case 'BinaryExpression':
                if (BIN[node.operator] === undefined) throw new Error(`Z no soporta operador ${node.operator}.`);
                this.emitExpr(node.left); this.emitExpr(node.right); this.emit(OPS.BIN, BIN[node.operator]); return;
            case 'LogicalExpression': {
                if (node.operator !== 'and' && node.operator !== 'or') throw new Error(`Z lógica inválida: ${node.operator}`);
                this.emitExpr(node.left); this.emit(OPS.DUP);
                const branch = this.jump(node.operator === 'and' ? OPS.JUMP_IF_FALSE : OPS.JUMP_IF_TRUE);
            ...
            case 'TableConstructorExpression': {
                ...
                for (const field of node.fields || []) {
                    ...
                    else if (field.type === 'TableValue') { this.emit(OPS.PUSH_CONST, this.program.number(arrayIndex++)); this.emitExpr(field.value); }
            ...
            case 'TableCallExpression': this.emitCall({ type: 'CallExpression', base: node.base, arguments: node.arguments }, wantMulti); return;
            case 'StringCallExpression': this.emitCall({ type: 'CallExpression', base: node.base, arguments: [node.argument] }, wantMulti); return;
    ...
    emitFunction(node, implicitSelf) {
        ...
        child.compileBlock(node.body || [], false); child.emit(OPS.PUSH_CONST, this.program.nil()); child.emit(OPS.RETURN, 1); child.resolveGotos(); this.emit(OPS.MAKE_FUNCTION, id);
    }

    emitLValueStore(node) {
        if (node.type === 'Identifier') return this.storeName(node.name);
        throw new Error('emitLValueStore se usa solo para identificadores.');
    }

    emitAssignmentTarget(node, tempKey) {
        if (node.type === 'Identifier') {
            this.emit(OPS.LOAD_VAR, this.program.string(tempKey));
    ...
    emitStatement(node) {
        ...
        switch (node.type) {
            case 'LocalStatement': {
                ...
                const keys = (node.variables || []).map(v => {
            ...
            case 'AssignmentStatement': {
                ...
                for (let i = 0; i < vars.length; i += 1) {
                    if (init[i]) this.emitExpr(init[i]); else this.emit(OPS.PUSH_CONST, this.program.nil());
                    const temp = this.allocTemp(); temps.push(temp); this.emit(OPS.STORE_VAR, this.program.string(temp));
                }
            ...
            case 'IfStatement': {
                const endJumps = [];
                for (let i = 0; i < (node.clauses || []).length; i += 1) { const clause = node.clauses[i]; if (clause.type === 'ElseClause') { this.compileBlock(clause.body, true); continue; } this.emitExpr(clause.condition); const no = this.jump(OPS.JUMP_IF_FALSE); this.compileBlock(clause.body, true); if (i < node.clauses.length - 1) endJumps.push(this.jump(OPS.JUMP)); this.patch(no, 1, this.here()); }
                const end = this.here(); for (const at of endJumps) this.patch(at, 1, end); return;
            }
            case 'WhileStatement': {
                const start = this.here(); this.emitExpr(node.condition); const exit = this.jump(OPS.JUMP_IF_FALSE); const loop = { breaks: [] }; this.loopStack.push(loop); this.compileBlock(node.body, true); this.emit(OPS.JUMP, start); const end = this.here(); this.patch(exit, 1, end); for (const at of loop.breaks) this.patch(at, 1, end); this.loopStack.pop(); return;
            }
            case 'RepeatStatement': {
                const start = this.here(); const loop = { breaks: [] }; this.loopStack.push(loop); this.compileBlock(node.body, true); this.emitExpr(node.condition); const back = this.jump(OPS.JUMP_IF_FALSE); this.patch(back, 1, start); const end = this.here(); for (const at of loop.breaks) this.patch(at, 1, end); this.loopStack.pop(); return;
            }
            case 'DoStatement': this.compileBlock(node.body, true); return;
            case 'BreakStatement': { const loop = this.loopStack[this.loopStack.length - 1]; if (!loop) throw new Error('Z encontró break fuera de un loop.'); const at = this.emit(OPS.BREAK); loop.breaks.push(at); return; }
            case 'ForNumericStatement': {
                ...
                const key = this.allocLocal(node.variable.name); const prep = this.emit(OPS.FOR_NUM_PREP, this.program.string(key)); const bodyStart = this.here(); const loop = { breaks: [] }; this.loopStack.push(loop);
                this.compileBlock(node.body, false); const next = this.emit(OPS.FOR_NUM_NEXT); this.emit(OPS.JUMP, bodyStart); const end = this.here(); this.patch(prep, 4, end); this.patch(next, 4, end);
                for (const at of loop.breaks) this.patch(at, 1, end); this.loopStack.pop(); this.popScope(); return;
            }
            case 'ForGenericStatement': {
                const vars = node.variables || []; const iterators = node.iterators || []; if (!vars.length || iterators.length !== 1) throw new Error('Z generic for soporta un solo iterador y al menos una variable.');
                this.emitExpr(iterators[0], true); this.pushScope(); const keys = vars.map(v => { if (v.type !== 'Identifier') throw new Error('Z generic for variable inválida.'); return this.allocLocal(v.name); });
                for (const key of keys) this.emit(OPS.PUSH_CONST, this.program.string(key));
                const prep = this.emit(OPS.ITER_PREP, keys.length); const bodyStart = this.here(); const loop = { breaks: [] }; this.loopStack.push(loop); this.compileBlock(node.body, false);
                const next = this.emit(OPS.ITER_NEXT, 0, bodyStart); const end = this.here(); this.patch(prep, 4, end); this.patch(next, 4, end); for (const at of loop.breaks) this.patch(at, 1, end); this.loopStack.pop(); this.popScope(); return;
            }
            case 'FunctionDeclaration': {
                const identifier = node.identifier; const implicitSelf = identifier && identifier.type === 'MemberExpression' && identifier.indexer === ':'; let localKey = null;
                if (node.isLocal && identifier.type === 'Identifier') localKey = this.allocLocal(identifier.name); this.emitFunction(node, !!implicitSelf);
...
function buildProgram(source) {
    const ast = luaparse.parse(source, { wait: false, comments: false, scope: false, locations: false, ranges: false, luaVersion: '5.1' });
    ...
    return { version: 3, root: rootId, constants: program.constants, functions: program.functions };
}
