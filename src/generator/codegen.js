...
class CodeGenerator {
    ...
    names() {
        const used = new Set();
        const fields = [
            'alphabet','map','parts','part','buf','chunk','program','pos','u16','u32','read','readValue',
            'constants','functions','ctype','size','fn','params','idx','globals','truth','getv','setv','gg','sg',
            'multi','invoke','packed','ok','exec','makeFn','stack','sp','env','varargs','loops','push','pop',
            'ins','op','a','b','c','d','v','key','obj','args','callArgs','result','left','right','frame','iter',
            'keys','start','finish','step','current','keep','code','def','ret','state','control','first','next'
        ];
        const out = {};
    ...
    decodeLines(packet, n) {
        ...
        return [
            ...
            `local ${n.a}=(${n.part}[4]+(((${n.idx}-1)/2)*${n.part}[6])+(((${n.idx}-1)/2)+1)*(((${n.idx}-1)/2)+${n.part}[5]))%256`,
            `local ${n.b}=(((${n.v}-${n.part}[7]-${n.a})%256)*${n.part}[9])%256`,
            `${n.chunk}[#${n.chunk}+1]=string.char(${n.b})`,
            'end',
    ...
    vmLines(program, n) {
        const C = program.constants.length;
        const F = program.functions.length;
        return [
    ...
    generate(source) {
        ...
        const n = this.names();
        const output = [...this.decodeLines(packet, n), ...this.vmLines(program, n)].join(' ');
